...
export default function MembersTable({ members }: { members: any[] }) {
  ...
  const filteredMembers = members.filter((member) => {
    const searchText =
      `${member.memberNumber} ${member.firstName} ${member.lastName} ${member.email}`.toLowerCase();
    ...
    <>
      <div className="mb-8">
        <input
          ...
          className="w-full rounded-2xl border border-[#ddd4c8] bg-white px-5 py-4 text-black shadow-sm placeholder:text-gray-400"
        />
      </div>
      ...
      <div className="grid gap-4 md:hidden">
        {filteredMembers.map((member) => (
          <div key={member.id} className="rounded-3xl bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 grid gap-3 text-sm">
              <p>
                <strong>Jahresbeitrag:</strong>{" "}
                {member.membershipFee.toFixed(2)} €
              </p>

              <div>
                <p className="mb-2 font-semibold">Beitrag:</p>
                <ContributionButton
                  memberId={member.id}
